...
function Gradient_descent_mul_var(X, y, alpha, lambda, num_iters, poly_degree)
...
n_new=size(X,2);
Theta = zeros(n_new, 1); % Almacena los valores finales de los parametros Theta
...
%     end
    ...
    Err=X * Theta - y;
    Theta(1)= Theta(1) - alpha * (1 / m ) * (( Err )' * X(:,1))';
    Theta(2:n_new)= Theta(2:n_new)*(1-alpha*lambda/m) - alpha * (1 / m ) * (( Err )' * X(:,2:n_new))';
    %disp(Theta);
end
